...
export function initTelemetry() {
  const serviceName = 'package-service';
  ...
  const prometheusExporter = new PrometheusExporter({
    port: 9465, // Different port from auth-service (9464)
    endpoint: '/metrics',
  });
  console.log('📊 Prometheus metrics exporter enabled on port 9465');
  ...
  console.log(`📊 Metrics available at: http://localhost:9465/metrics`);
